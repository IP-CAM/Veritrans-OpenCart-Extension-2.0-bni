...
class ControllerPaymentVeritransbni extends Controller {
  ...
  public function index() {
    ...
    $data['button_confirm'] = $this->language->get('button_confirm');
  	...
  	$data['pay_type'] = $this->config->get('veritransbni_payment_type');
    $data['text_loading'] = $this->language->get('text_loading');

  	$data['process_order'] = $this->url->link('payment/veritransbni/process_order');
  ...
  public function process_order() {
    ...
    $this->model_checkout_order->addOrderHistory($this->session->data['order_id'],
        $this->config->get('veritransbni_vtweb_challenge_mapping'));
    ...
    if ($this->config->get('config_currency') != 'IDR') {
      ...
      else if ($this->config->get('veritransbni_currency_conversion') > 0) {
        foreach ($item_details as &$item) {
          $item['price'] = intval($item['price']
              * $this->config->get('veritransbni_currency_conversion'));
        }
        unset($item);
    ...
    Veritrans_Config::$serverKey = $this->config->
        get('veritransbni_server_key_v2');

    Veritrans_Config::$isProduction =
        $this->config->get('veritransbni_environment') == 'production'
        ? true : false;
    ...
    try {
      ...
      else if ($this->config->get('veritransbni_installment_option') == 'certain_product')
      {
  ...
  public function landing_redir() {
    ...
    if( isset($_GET['order_id']) && isset($_GET['transaction_status']) && ($_GET['transaction_status'] == 'capture' || $_GET['transaction_status'] == 'pending' || $_GET['transaction_status'] == 'settlement')) {
      ...
      $redirUrl = $this->url->link('payment/veritransbni/failure','','SSL');
      $this->response->redirect($redirUrl);
  ...
  */
  public function failure() {
    ...
    if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/veritransbni_checkout_failure.tpl')) {
      $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/veritransbni_checkout_failure.tpl', $data));
    } else {
      $this->response->setOutput($this->load->view('default/template/payment/veritransbni_checkout_failure.tpl', $data));
    }
  }
}
